...
namespace Shopping_Cart_2.Services
{
    public class RatingService :IRatingService
    {
        ...
        {
             var rated = _db.Ratings?.Where(r => r.UserId == userId && r.ItemId==itemId).FirstOrDefault();
            if (rated != null)
            {
        ...
        {
            var rating = _db.Ratings.Where(r => r.UserId == userId && r.ItemId == itemId).SingleOrDefault();
            if (rating == null)
            {
                return 0;
            }
        ...
        {
            var rate = _db.Ratings?.Where(r => r.UserId == userId && r.ItemId == itemId).SingleOrDefault();
            if (rate == null)
            {
                return -1;
            }
        ...
        {
            var productRate = _db.Ratings.Where(r => r.ItemId == itemId).ToList();
            if (productRate.Count()==0 ) { return -1; }
